    ...
    useEffect(() => {
        ...
        async function fetchDetail() {
            try {
                ...
                setName(data.design.name);
                setBedRoom(data?.design.room.bedRoom);
                setRestRoom(data?.design.room.restRoom);
                setFloor(data?.design.floor);
                setDescription(data?.design.description);
                let quote = data.size.long *
                    data.size.wide *
                    (currentPart.rawPart +
                        currentPart.finishingPart);
                if (data?.design.floor != 1) {
                    quote = quote * ((data?.design.floor - 1) * 0.7 + 1);
                }
                if (pictures.length == 0) {
                    setPictures(data?.design.pictures);
                }
                setSelectedSize(data?.size);
                setTotal(quote);
                setPaintWall(data?.design.paintWallQuantity);
                setRoof(data?.design.roofQuantity);
                setDoor(data?.design.doorQuantity);
                setWindow(data?.design.windowQuantity);
                setWallTitle(data?.design.wallTitleQuantity);
                setFloorTitle(data?.design.floorTitleQuantity);
    ...
    const handleRemove = async () => {
        try {
            const response = await axios.delete(
                `http://localhost:3000/api/design/${designDetail._id}`,
                {
                    headers: { token: `Bearer ${currentUser?.accessToken}` },
                });
            console.log('Design save updated successfully:', response.data);
    ...
    return (
        <div className="mt-8" id="AdminDesignDetail">
            <div className="text-4xl font-bold flex justify-center">
                Design Details - <TextField id="outlined-basic" label="Name" variant="outlined" value={name} onChange={(e) => setName(e.target.value)} />
            </div>
            {designDetail ? (
                <div className="flex">
                    ...
                    <div className="flex gap-10 flex-wrap mt-14 ml-16">
                        <div className="w-[250px] h-36">
                            ...
                            <div className="mt-3 pb-5">
                                <div className="flex gap-2 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    <div>Pain Wall:
                                    </div>
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={paintWall} onChange={(e) => setPaintWall(e.target.value)} />
                                </div>
                                <div className="flex gap-2 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    <div>Roof:
                                    </div>
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={roof} onChange={(e) => setRoof(e.target.value)} />
                                </div>
                                <div className="flex gap-3 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    <div>Door:
                                    </div>
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={door} onChange={(e) => setDoor(e.target.value)} />
                                </div>
                                <div className="flex gap-3 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    <div>Window:
                                    </div>
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={window} onChange={(e) => setWindow(e.target.value)} />
                                </div>
                                <div className="flex gap-3 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    <div>Wall Title:
                                    </div>
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={wallTitle} onChange={(e) => setWallTitle(e.target.value)} />
                                </div>
                                <div className="flex gap-3 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    <div>Floor Title:
                                    </div>
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={floorTitle} onChange={(e) => setFloorTitle(e.target.value)} />
                                </div>
                            </div>
                        </div>
                        {designDetail.size && (
                            <div>
                                ...
                                <div className="flex gap-3 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    ...
                                    <p><TextField id="outlined-basic" label="Name" variant="outlined" value={total} onChange={(e) => setTotal(e.target.value)} /> </p>
                                </div>
                                <div className="flex gap-3 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                        ...
                                        >
                                            {
                                                sizes && sizes.map((item, index) => {
                                                    if (item._id == selectedSize?._id) {
                                                        return (
                                                            <MenuItem key={index} value={selectedSize}>{selectedSize.long * selectedSize.wide} m2 -
                                                                R{selectedSize.wide} X R{selectedSize.long}</MenuItem>
                                                        )
                                                    } else {
                                                        return (
                                                            <MenuItem key={index} value={item}>{item.long * item.wide} m2 -
                                                                R{item.wide} X R{item.long}</MenuItem>
                                                        )
                                                    }
                                                })
                                            }
                                ...
                                <div className="flex gap-2 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    ...
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={bedRoom} onChange={(e) => setBedRoom(e.target.value)} />
                                </div>
                                <div className="flex gap-2 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    <FontAwesomeIcon icon={faToilet} />
                                    <div>Restrooms:
                                    </div>
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={restRoom} onChange={(e) => setRestRoom(e.target.value)} />
                                </div>
                                <div className="flex gap-3 mb-1" style={{ alignItems: "center", justifyContent: "space-between" }}>
                                    <FontAwesomeIcon icon={faHouse} />
                                    <div>Floors:
                                    </div>
                                    <TextField id="outlined-basic" variant="outlined" style={{ maxWidth: "50px" }} value={floor} onChange={(e) => setFloor(e.target.value)} />
                                </div>
